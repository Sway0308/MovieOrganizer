...
namespace MovieOrganizer.Adaptors
{
    public class FilmAdaptor
    {
        ...
        public void Categorize(string path)
        {
            ...
            Console.WriteLine($"All films count: {Handler.Films.Count}");
            if (Handler.Films.Count == 0)
                return;
        ...
        {
            var props = typeof(FilmModel).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
            var title = string.Join(",", props.Select(x => x.Name));

            var content = new string[props.Length];
            var str = new StringBuilder();
            str.AppendLine(title);
            foreach (var item in Handler.Films)
            {                
